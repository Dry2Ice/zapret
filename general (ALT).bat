...
for %%F in ("%BIN%quic_initial_www_google_com.bin" "%BIN%quic_initial_dbankcloud_ru.bin" "%BIN%tls_clienthello_www_google_com.bin" "%BIN%stun.bin" "%BIN%tls_clienthello_max_ru.bin") do (
    if not exist "%%F" (
