...
Запустите **`service.bat`**, выберите пункт **`Update hosts file`**. После чего, если ваш hosts будет неактуальным, то Вам будет предложено обновить его самостоятельно:  
  ...
  - Если сайты/сервисы «крутятся» бесконечно, временно отключите policy-layer: удалите файл `utils/policy.enabled`, запустите `general.bat` и проверьте базовую стратегию без `zapret: policy`. Для обратного включения создайте пустой файл `utils/policy.enabled` снова.
...
- Быстрая диагностика: откройте `service.bat` и проверьте, что `IPSet Filter` не находится в режиме `any` (в меню отображается как `any (danger)`). Если стоит `any`, переключите на `none` или `loaded` и проверьте снова.
